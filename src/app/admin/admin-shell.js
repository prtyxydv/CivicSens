...
export default function AdminShell({ children }) {
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem("civicsens_theme");
  ...
  return (
    <ThemeContext.Provider value={{ isDark, toggleTheme }}>
      <div
        className={`min-h-screen flex overflow-hidden font-sans transition-colors duration-500 ${
      ...
      >
        ...
        >
          <div className="p-6 flex items-center justify-between">
            <div className={`flex items-center gap-3 ${!isSidebarOpen && "hidden"}`}>
              ...
              <div>
                ...
                <p className="text-[8px] font-bold tracking-[0.2em] uppercase opacity-50">
                  Admin Center
                </p>
              </div>
            </div>
          ...
          <div className={`p-4 border-t ${isDark ? "border-slate-800" : "border-slate-200"}`}>
            <button
              onClick={toggleTheme}
              className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all mb-2 ${
                isDark
            ...
            >
              ...
              <span className={`font-bold text-sm tracking-widest uppercase ${!isSidebarOpen && "hidden"}`}>
        ...
        <main className="flex-1 overflow-y-auto relative">
          <div
            className={`absolute top-0 left-0 w-full h-[500px] pointer-events-none transition-opacity duration-1000 ${
              isDark
                ? "bg-gradient-to-b from-blue-600/10 to-transparent opacity-100"
                : "bg-gradient-to-b from-blue-500/5 to-transparent opacity-100"
            }`}
          />
...
function NavItem({ icon, label, active = false, isOpen = true, isDark = true }) {
  return (
    ...
    >
      <span
        className={`${
          active ? "text-white" : isDark ? "text-slate-400 group-hover:text-blue-400" : "text-slate-500 group-hover:text-blue-600"
        } transition-colors`}
      >
        {icon}
      </span>
      ...
      {active && isOpen && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-white shadow-[0_0_8px_white]" />}
    </button>
  );
}
